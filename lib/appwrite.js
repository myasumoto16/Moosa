...
export const getSavedPosts = async (userId) => {
  // console.log('userId in getSavedPosts: ' + userId)
  try {
      ...
      const videoIds = likesResponse.documents.map(c => c.videoId)
...
export const isVideoLikedByUser = async (userId, videoId) => {
  try {
      ...
      // Check if the documents array is not empty
      if (likeCheck.documents && likeCheck.documents.length > 0) {
          console.log('Like exists for userId:', userId, 'and videoId:', videoId);
          return [true, likeCheck.documents[0].$id]; // Like exists
      } else {
          console.log('No like found for userId:', userId, 'and videoId:', videoId);
          return [false, null]; // Like does not exist 
      }
  } catch (error) {
...
export const addLike = async (videoId, userId) => {
  try {
    console.log('addLike', videoId, userId)
    ...
    throw new Error('error when adding a like', error);
  }
}

export const removeLike = async (likesDocId) => {
  try {
    console.log("likesdocId: " + likesDocId)
    ...
    throw new Error('error when removing a like', error);
  }
}


export const updateLike = async (like, videoId, userId, likesDocId) => {
  try {
    if (like) {
      await addLike(videoId, userId)
    }
    else {
      await removeLike(likesDocId)
    }
  } catch (error) {
    throw new Error(error);
  }
